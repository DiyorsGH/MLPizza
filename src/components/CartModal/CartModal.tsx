...
export default function CartModal({
    ...
    return (
        ...
        <div className="w-full h-[10%] flex justify-between items-center p-4">
            ...
            ref={closeBtnRef}
            className="closeBtn absolute top-4 right-4 text-large"
            >
            ×
            </button>
        </div>
        ...
        {cartItems.length === 0 ? (
            ...
            <button
                className="orangeBtn-styles px-5 py-3 mt-4 text-medium"
                onClick={() => {
                const dialog = dialogRef.current;
                if (!dialog) return;
                if (typeof (dialog as any)._fadeOutAndClose === "function") {
            ...
            <div className="w-full h-[26%] bg-white flex flex-col items-center justify-center p-4 border-t-2 border-[var(--gray-dark)] gap-10">
                ...
                <button
                className="orangeBtn-styles w-full py-3 text-medium"
                onClick={() => {
                    const dialog = dialogRef.current;
                    if (!dialog) return;
                    if (typeof (dialog as any)._fadeOutAndClose === "function") {
